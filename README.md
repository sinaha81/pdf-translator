...
This web tool allows you to translate SubRip Subtitle (.pdf) files or pasted pdf content into another language using Google's Gemini AI models. The translation process happens entirely in your browser, requiring your own Gemini API key.
...
- Translate `.pdf` files or pasted pdf text.
...
2.  **Select Input Method:**
    - **Upload File:** Click the "Upload File" radio button, then drag & drop your `.pdf` file onto the designated area or click it to browse.
    - **Paste Text:** Click the "Paste Text" radio button and paste your complete pdf content into the text area.
...
5.  **(Optional) Configure Advanced Settings:** Expand the "Settings & API Key" section to adjust:
    ...
    - **Proxy:** Enable if you are in a region where direct access to the Gemini API is blocked (like Iran). _Requires the proxy URL to be correctly configured._
    ...
    - **Chunks:** Set how many parts the pdf file should be split into for processing. Higher numbers can sometimes help with very long files or strict rate limits, but increase overhead.
...
8.  **Download / Retry:**
    - Once complete, a "Download Translated pdf" button will appear. Click it to save the file.
...
- **API Key Security:** Your API key is processed in your browser and sent directly to Google (or the proxy). While not stored on any external server by _this tool_, be mindful of browser extensions or local security. Use the "Remember Me" feature cautiously.
...
- ترجمه فایل‌های `.pdf` یا متن pdf الصاق شده.
...
2.  **انتخاب روش ورودی:**
    - **بارگذاری فایل:** دکمه رادیویی «بارگذاری فایل» را انتخاب کنید، سپس فایل `.pdf` خود را به قسمت مشخص شده بکشید و رها کنید یا روی آن کلیک کنید تا فایل را انتخاب نمایید.
    - **الصاق متن:** دکمه رادیویی «الصاق متن» را انتخاب کنید و محتوای کامل pdf خود را در کادر متنی مربوطه الصاق (Paste) کنید.
...
5.  **(اختیاری) پیکربندی تنظیمات پیشرفته:** بخش «تنظیمات پیشرفته» را باز کنید تا موارد زیر را تنظیم نمایید:
    ...
    - **پروکسی:** اگر در منطقه‌ای هستید که دسترسی مستقیم به Gemini API مسدود است (مانند ایران)، این گزینه را فعال کنید. _نیاز به پیکربندی صحیح URL پروکسی دارد._
    ...
    - **تعداد بخش‌ها:** تعیین کنید که فایل pdf برای پردازش به چند قسمت تقسیم شود. اعداد بالاتر گاهی اوقات می‌توانند به فایل‌های بسیار طولانی یا محدودیت‌های شدید نرخ ارسال کمک کنند، اما سربار را افزایش می‌دهند.
...
8.  **دانلود / تلاش مجدد:**
    - پس از اتمام، دکمه «دانلود فایل pdf» ظاهر می‌شود. برای ذخیره فایل روی آن کلیک کنید.
...
- **امنیت کلید API:** کلید API شما در مرورگر شما پردازش شده و مستقیماً به Google (یا پروکسی) ارسال می‌شود. اگرچه توسط _این ابزار_ در هیچ سرور خارجی ذخیره نمی‌شود، مراقب افزونه‌های مرورگر یا امنیت محلی باشید. از ویژگی «ذخیره کلید API» با احتیاط استفاده کنید.
